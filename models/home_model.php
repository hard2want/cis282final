...
$strSQL = "SELECT ch.cohort_id, t.code, term.year, term.semester, t.days, t.start_time, t.minutes, count(u.id) AS numOfStudents
FROM cis282final.times t, cis282final.cohorts ch, cis282final.courses c, cis282final.terms term, cis282final.users u
WHERE ch.time_id = t.time_id AND ch.term_id = term.term_id AND u.cohort_id = ch.cohort_id
GROUP BY u.cohort_id
ORDER BY ch.cohort_id"; // WHERE u.user_id = $userId
...
$classData = mysqli_fetch_all($result, MYSQLI_ASSOC); // you will rename your variable for each specific query




/*
$strSQL = "SELECT count(id) as numOfStudents
FROM cis282final.users
...
// Step 2 - FETCH DATA: 
$studentCount = mysqli_fetch_all($result, MYSQLI_ASSOC); // you will rename your variable for each specific query
*/
